...
//	public SipManager(SipProfile sipProfile, boolean connectivity) {
	...
	public void Cancel() throws NotInitializedException
	{
		...
		if (direction == CallDirection.OUTGOING || direction == CallDirection.INCOMING) {
			if (currentClientTransaction != null) {
	...
	public void processResponse(ResponseEvent arg0) {
		...
		if (response.getStatusCode() == Response.PROXY_AUTHENTICATION_REQUIRED
			...
			if (cseq.getMethod().equals(Request.INVITE)) {
				RCLogger.i(TAG, "Ravi Dialog after 200 OK  " + dialog);
				try {
					RCLogger.i(TAG, "RAvi Sending ACK");

				/*	dialog = arg0.getDialog();
					CSeqHeader cSeq = (CSeqHeader) arg0.getResponse().getHeader(CSeqHeader.NAME);
					RCLogger.i(TAG, "RAvi CSeq Received "+cSeq.toString());
					Request ackRequest = dialog.createAck(cSeq.getSeqNumber());
					System.out.println("Ack Message  " + ackRequest.toString());
